...
const WarehouseAudit = () => {
  ...
  const filteredDetails = auditFormMode === 'edit' && auditForm?.details
    ? auditForm.details.filter(d =>
        !medSearchQuery || d.medicineName?.toLowerCase().includes(medSearchQuery.toLowerCase())
      )
    : [];
          ...
          <div className="content-card" style={{ width: '98%', height: '128vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#ffffff', border: '1px solid #cbd5e1', padding: '24px', borderRadius: '8px', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.15)' }}>
            ...
            {auditFormMode === 'edit' ? (
              <form onSubmit={handleSaveDraftQuantity} style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
                ...
                <div className="form-group" style={{ flexShrink: 0, marginBottom: '15px' }}>
                  <label className="label">Tìm kiếm thuốc trong phiếu:</label>
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault();
                          setMedSearchQuery(medSearchInput);
                        }
                      }}
                    />
                    <button
                      type="button"
                      className="btn-create"
                      style={{ backgroundColor: 'var(--success-color)', whiteSpace: 'nowrap', margin: 0, padding: '10px 20px' }}
                      onClick={() => setMedSearchQuery(medSearchInput)}
                    >
                      Tìm kiếm
                    </button>
                    {medSearchInput && (
                      <button
                        type="button"
                        className="btn-create"
                        style={{ backgroundColor: '#64748b', whiteSpace: 'nowrap', margin: 0, padding: '10px 20px' }}
                        onClick={() => {
                          setMedSearchInput('');
                          setMedSearchQuery('');
                        }}
